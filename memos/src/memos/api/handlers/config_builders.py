...
def build_vec_db_config() -> dict[str, Any] | None:
    """
    Build vector database configuration.

    Returns:
        Validated vector database configuration dictionary, or None if preference memory is disabled
    """
    # Only build vector DB config if preference memory is enabled
    if os.getenv("ENABLE_PREFERENCE_MEMORY", "false").lower() != "true":
        return None
    
    # Use Qdrant by default, or Milvus if explicitly configured
    vec_db_backend = os.getenv("VEC_DB_BACKEND", "qdrant").lower()
    
    if vec_db_backend == "qdrant":
